...
"""
Matches representatives data to add URL and picture.
"""
...
def match(representatives, lastnames, row):
    name, url, pic = row
    k = slugify(name)
    i = slugify(name.split(',', 2)[0].strip())
    if k in representatives:
        representatives[k]['url'] = url
        representatives[k]['foto'] = pic
        ...
        for j in lastnames[i]:
            ...
            if confirm(prompt):
                representatives[j['id']]['url'] = url
                representatives[j['id']]['foto'] = pic
    else:
...
def main(program, filename=None, *args):
    ...
    for row in load('urls.csv'):
        try:
            match(representatives, lastnames, row)
        except EOFError:
            break
    logging.debug('Done!')
